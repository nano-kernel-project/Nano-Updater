...
import com.codebot.axel.kernel.updater.model.NanoPackage
...
class Utils {
    ...
    fun isUpdateAvailable(context: Context, nanoData: Nano?, buildDate: String, animation: RotateAnimation) {
        ...
        when {
            /*Constants.DEVICE in supportedDevices && !isKernelInstalled -> {
                ...
                verifyAndDisplayUpdate(context, nanoPackage)
            }
            buildDate == "" && !isKernelInstalled -> {
                context.update_notify_textView.text = "Unsupported device"
                context.update_notify_textView.setTextColor(ContextCompat.getColor(context, R.color.accentTitleColor))
                context.update_notify_textView.compoundDrawableTintList = ColorStateList.valueOf(ContextCompat.getColor(context, R.color.accentTitleColor))
                context.update_notify_textView.setCompoundDrawablesWithIntrinsicBounds(context.getDrawable(R.drawable.ic_info), null, null, null)
            }*/
    ...
    fun setViewVisibilityAndListeners(context: Context, installPackage: File, compactLayoutVisibility: Int) {
        ...
        if (flashLayoutVisibility == View.VISIBLE) {
            val layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
            val bottomSheetBehavior = BottomSheetBehavior.from(context.bottom_sheet)
            layoutParams.setMargins(0, 0, 0, getPaddingUnitsInDp(context, bottomSheetBehavior!!.peekHeight + 16))
     ...
     */
    fun getBuildProperty(propKey: String): String {
        var propValue = ""
        try {
                    ...
                    if (files!!.isNotEmpty()) {
                        for (file in files) {
                            ...
                            if (file.name == nanoPackage[0].filename) {
                                ...
                                context.packageInfoCompact.visibility = View.GONE
                                context.packageInfoExpanded.visibility = View.GONE
    ...
    fun getPaddingUnitsInDp(context: Activity, paddingValue: Int): Int {
        val scale = context.resources.displayMetrics.density
        return (paddingValue * scale + 0.5f).toInt()
    }
}
